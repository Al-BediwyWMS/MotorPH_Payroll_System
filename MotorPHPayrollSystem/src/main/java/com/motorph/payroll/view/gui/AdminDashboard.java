...
public class AdminDashboard extends JFrame {
    ...
    private void showPayrollGeneration() {
        ...
        // Main content
        JPanel payrollContent = new JPanel(new BorderLayout());
        payrollContent.setOpaque(false);
        payrollContent.setBorder(new EmptyBorder(20, 0, 0, 0));
        
        // Employee selection
        JPanel employeeSelectionPanel = new JPanel(new BorderLayout());
        employeeSelectionPanel.setOpaque(false);
        employeeSelectionPanel.setBorder(new EmptyBorder(0, 0, 20, 0));
        ...
        // Options panel
        RoundedPanel optionsPanel = new RoundedPanel(new BorderLayout(), new Color(255, 255, 255, 220));
        optionsPanel.setBorder(new EmptyBorder(15, 15, 15, 15));
        
        JLabel optionsTitle = new JLabel("Payroll Options");
        optionsTitle.setFont(new Font("Montserrat", Font.BOLD, 16));
        
        JPanel optionsContentPanel = new JPanel(new GridLayout(3, 2, 15, 10));
        optionsContentPanel.setOpaque(false);
        optionsContentPanel.setBorder(new EmptyBorder(10, 0, 0, 0));
        
        JLabel taxRateLabel = new JLabel("Tax Rate (%):");
        taxRateLabel.setFont(new Font("Montserrat", Font.PLAIN, 14));
        JTextField taxRateField = new JTextField("12.0");
        taxRateField.setFont(new Font("Montserrat", Font.PLAIN, 14));
        
        JLabel sssRateLabel = new JLabel("SSS Rate (%):");
        sssRateLabel.setFont(new Font("Montserrat", Font.PLAIN, 14));
        JTextField sssRateField = new JTextField("4.5");
        sssRateField.setFont(new Font("Montserrat", Font.PLAIN, 14));
        
        JLabel philhealthRateLabel = new JLabel("PhilHealth Rate (%):");
        philhealthRateLabel.setFont(new Font("Montserrat", Font.PLAIN, 14));
        JTextField philhealthRateField = new JTextField("1.5");
        philhealthRateField.setFont(new Font("Montserrat", Font.PLAIN, 14));
        
        optionsContentPanel.add(taxRateLabel);
        optionsContentPanel.add(taxRateField);
        optionsContentPanel.add(sssRateLabel);
        optionsContentPanel.add(sssRateField);
        optionsContentPanel.add(philhealthRateLabel);
        optionsContentPanel.add(philhealthRateField);
        ...
        ModernButton generateButton = new ModernButton("Generate Payslips");
        ...
        generateButton.addActionListener(e -> {
            ...
            int confirm = JOptionPane.showConfirmDialog(this,
                "Generate payslips for " + selectedCount + " employee(s)?",
                "Confirm Generation",
                JOptionPane.YES_NO_OPTION);
                
            if (confirm == JOptionPane.YES_OPTION) {
                JOptionPane.showMessageDialog(this,
                    "Payslips generated successfully for " + selectedCount + " employee(s).",
                    "Success",
        ...
        // Add components to payroll content
        payrollContent.add(employeeSelectionPanel, BorderLayout.NORTH);
        payrollContent.add(optionsPanel, BorderLayout.CENTER);
        payrollContent.add(buttonPanel, BorderLayout.SOUTH);
        
        // Add components to payroll panel
        payrollPanel.add(titlePanel, BorderLayout.NORTH);
        payrollPanel.add(payrollContent, BorderLayout.CENTER);
    ...
    private void showAttendanceManagement() {
        ...
        employeeLabel.setFont(new Font("Montserrat", Font.PLAIN, 14));
        
        // Create employee dropdown
        List<Employee> employees = employeeController.getAllEmployees();
        String[] employeeOptions = new String[employees.size() + 1];
        employeeOptions[0] = "All Employees";
        for (int i = 0; i < employees.size(); i++) {
            Employee employee = employees.get(i);
            employeeOptions[i + 1] = employee.getEmployeeId() + " - " + employee.getFirstName() + " " + employee.getLastName();
        }
        
        JComboBox<String> employeeComboBox = new JComboBox<>(employeeOptions);
        employeeComboBox.setFont(new Font("Montserrat", Font.PLAIN, 14));
        ...
        dateComboBox.setFont(new Font("Montserrat", Font.PLAIN, 14));
        
        ModernButton viewButton = new ModernButton("View");
        viewButton.setPreferredSize(new Dimension(100, 30));
        
        controlsPanel.add(employeeLabel);
        controlsPanel.add(employeeComboBox);
        controlsPanel.add(Box.createHorizontalStrut(10));
        controlsPanel.add(dateLabel);
        controlsPanel.add(dateComboBox);
        controlsPanel.add(Box.createHorizontalStrut(10));
        controlsPanel.add(viewButton);
        ...
        JLabel dateRangeLabel = new JLabel("Showing attendance for: Today (" + 
        ...
        // Table panel
        String[] columnNames = {"Employee ID", "Name", "Date", "Time In", "Time Out", "Total Hours", "Status"};
        DefaultTableModel model = new DefaultTableModel(columnNames, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
        ...
        // Sample data
        model.addRow(new Object[]{"1001", "Dela Cruz, Juan", "05/28/2025", "08:03 AM", "05:02 PM", "8.0", "Present"});
        model.addRow(new Object[]{"1002", "Santos, Maria", "05/28/2025", "08:15 AM", "05:05 PM", "7.8", "Present"});
        model.addRow(new Object[]{"1003", "Reyes, Roberto", "05/28/2025", "08:35 AM", "05:10 PM", "7.6", "Late"});
        model.addRow(new Object[]{"1004", "Garcia, Ana", "05/28/2025", "08:00 AM", "05:00 PM", "8.0", "Present"});
        model.addRow(new Object[]{"1005", "Tan, Michael", "05/28/2025", "08:45 AM", "05:15 PM", "7.5", "Late"});
        ...
        ModernButton addButton = new ModernButton("Add Attendance");
        addButton.setPreferredSize(new Dimension(150, 35));
        addButton.addActionListener(e -> showAddAttendanceForm());
        ...
        editButton.addActionListener(e -> {
            ...
            if (selectedRow >= 0) {
                showEditAttendanceForm(selectedRow);
            } else {
                JOptionPane.showMessageDialog(this,
        ...
        ModernButton exportButton = new ModernButton("Export");
        ...
        attendanceContent.add(dateRangePanel, BorderLayout.NORTH);
        attendanceContent.add(scrollPane, BorderLayout.CENTER);
        attendanceContent.add(actionPanel, BorderLayout.SOUTH);
    ...
    private void showEmployeeManagement() {
        showEmployeeList(); // Redirect to employee list for simplicity
    }
    ...
    private void showAddEmployeeForm() {
        ...
        personalPanel.add(departmentField);
        ...
        JTextField clothingField = new JTextField();
        ...
        saveButton.addActionListener(e -> {
            ...
            // Create new employee with all required parameters
            try {
                int newId = employeeController.getNextEmployeeId();
                Employee newEmployee = new Employee(
                    ...
                    parseDoubleOrDefault(salaryField.getText(), 0.0),
                    parseDoubleOrDefault(riceField.getText(), 0.0),
                    parseDoubleOrDefault(phoneAllowanceField.getText(), 0.0),
                    parseDoubleOrDefault(clothingField.getText(), 0.0),
                    calculateGrossSemiMonthly(parseDoubleOrDefault(salaryField.getText(), 0.0)),
                    calculateHourlyRate(parseDoubleOrDefault(salaryField.getText(), 0.0))
                );
                
                // Add employee (in a real app, this would save to database)
